...
export const BrandVisionSection = (): JSX.Element => {
  ...
  const [counters, setCounters] = useState({
    first: 0,   // 95%
    second: 0,  // 150%
    third: 0,   // 1.0 (will display as 1.0M+)
  });
  ...
  const animateCounter = (
    ...
    new Promise<void>((resolve) => {
      ...
      const tick = (now: number) => {
        ...
        // For decimals (like 1.0): keep 1 decimal place smoothly
        if (decimals > 0) {
    ...
    const observer = new IntersectionObserver(
      async (entries) => {
        ...
        if (entry.isIntersecting && !hasAnimated.current) {
          ...
          // 3) 0.0 -> 1.0 (one decimal) — shown as "1.0M+"
          await animateCounter(1.0, 900, (v) =>
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-12 items-start mb-6">
          ...
          <div className="lg:col-span-5 lg:w-[600px]">
            <div className="relative rounded-2xl overflow-hidden shadow-lg ">
              <img
                src="/theimpulsedigital.com/Cover Image.jpg"
                alt="SEO workspace"
