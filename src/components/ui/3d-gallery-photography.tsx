...
const MAX_HORIZONTAL_OFFSET = 24;
const MAX_VERTICAL_OFFSET = 18;
...
function GalleryScene({
	...
	const spatialPositions = useMemo(() => {
		...
		// Hand-crafted positions to ensure they stay on-screen, fill empty space, and don't overlap
		const xSpread = [ -10, 10, -5, 5, -12, 12, 0, -8, 8, -11, 11, 0 ];
		const ySpread = [ -2, 2, 5, -5, 0, 0, 7, -7, -3, 3, -6, 6 ];

		for (let i = 0; i < visibleCount; i++) {
			const x = xSpread[i % xSpread.length];
			const y = ySpread[i % ySpread.length];
			positions.push({ x, y });
		}
	...
	return (
		<>
			{planesData.current.map((plane, i) => {
				...
				const originalImage = normalizedImages[plane.imageIndex];
